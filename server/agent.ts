...
FOCUS
Draw the key elements this issue is about at full fidelity. Everything around them — the game scene or page behind them, backdrops, panels or HUD this issue doesn't change — is context: render it as a flat neutral placeholder (a single mid-grey fill with a small "context" label), never detailed art, and mark its container data-mockup-context. The app gets checked against this mockup automatically, so any detail you paint into the context becomes something the implementation is judged on.
...
A flat mid-grey area labelled "context" is a context placeholder: the mockup deliberately left that part of the screen out of scope. Never judge what fills it on either image (a game scene, a backdrop, other UI), and never name it as the gap.
